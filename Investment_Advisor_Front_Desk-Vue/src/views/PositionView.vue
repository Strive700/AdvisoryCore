    ...
    <div class="position-section">
      ...
      <div class="table-container">
        <table class="data-table" id="position-table">  
          <thead>
            <tr>
              <th class="align-center">基金代码</th>
              <th class="align-center">基金名称</th>
...
function getStockName(tsCode: string) {
  return names.value[tsCode]?.name || '未知基金'
}
...
function sellStock(pos: Position) {
  ...
  sendPositionTrack('position_sell_click', '点击卖出', { tsCode: pos.tsCode, stockName: getStockName(pos.tsCode) }, '卖出按钮', '.btn-sell')
}
function onPageClick(p: number) {
  fetchPositions(p)
  sendPositionTrack('position_page_change', '切换持仓分页', { page: p }, '分页按钮', '.page-btn.active')
}
onMounted(() => {
  fetchPositions(1)
})
