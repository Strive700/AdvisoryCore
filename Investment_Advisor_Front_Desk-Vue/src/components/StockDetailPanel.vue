...
function sendTabTrack(tabName: string) {
  trackEvent({
    event_id: tabName === 'company' ? 'view_company_info' : 'view_business_info',
    event_name: tabName === 'company' ? '查看公司资料' : '查看业务信息',
    module: 'stock_detail_panel',
    page_name: '股票详情',
    trigger_type: 'click',
    target_component: tabName === 'company' ? '公司资料Tab' : '业务信息Tab',
    selector: tabName === 'company' ? '#company-info' : '#business-info',
    extra_params: {
      tsCode: props.tsCode,
      stockName: detail.value?.name || '',
      stockCode: detail.value?.tsCode || ''
    },
...
function sendOpenTrack() {
  trackEvent({
    event_id: 'open_stock_detail_panel',
    event_name: '打开股票详情面板',
    module: 'stock_detail_panel',
    page_name: '股票详情',
    trigger_type: 'open',
    target_component: '股票详情面板',
    selector: '#stock-detail-panel',
    extra_params: {
      tsCode: props.tsCode,
      stockName: detail.value?.name || '',
      stockCode: detail.value?.tsCode || ''
