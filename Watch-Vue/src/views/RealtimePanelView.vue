...
async function fetchTrendData() {
  ...
  const [userTrend, eventTrend] = await Promise.all([
    request.get('/track/event/active-users-trend?minutes=10').then(r => r.data),
    request.get('/track/event/event-count-trend?minutes=10').then(r => r.data),
  ])
  // 格式化数据
  console.log('活跃用户趋势数据:', userTrend)
  console.log('事件数趋势数据:', eventTrend)
