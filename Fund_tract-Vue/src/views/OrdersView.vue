  ...
  <div class="orders-page">
    ...
    <div class="filter-section">
      <div class="filter-row">
        ...
        <div class="filter-group">
          <label>基金代码</label>
          <input v-model="filters.tsCode" type="text" placeholder="请输入基金代码" />
        </div>
        <div class="filter-group">
          <label>交易类型</label>
...
function resetFilters() {
  ...
  fetchOrders(1)
  sendOrdersTrack('orders_reset', '重置订单筛选', {}, '重置按钮', '.btn-default')
}
async function cancelOrder(orderId: string) {
  ...
  try {
    ...
    sendOrdersTrack('orders_cancel', '取消订单', { orderId }, '取消订单按钮', '.btn-cancel')
  } finally {
    isLoading.value = false
  }
}
function onPageClick(p: number) {
  fetchOrders(p)
  sendOrdersTrack('orders_page_change', '切换订单分页', { page: p }, '分页按钮', '.page-btn.active')
}
onMounted(() => {
  fetchOrders(1)
})
