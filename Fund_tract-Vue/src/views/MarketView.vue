  ...
  <div class="market-page">
    ...
    <div class="market-filter">
      <div class="filter-group">
        <input v-model="searchKeyword" type="text" placeholder="输入基金名称搜索" class="search-input" @keyup.enter="onSearch" />
    ...
    <div v-if="total > 0" class="pagination">
      <button class="page-btn" :disabled="page === 1" @click="onPageClick(page - 1)">«</button>
      <button v-for="p in visiblePages" :key="p" class="page-btn" :class="{ active: p === page }" @click="onPageClick(p)">{{ p }}</button>
      <button class="page-btn" :disabled="page === totalPages" @click="onPageClick(page + 1)">»</button>
...
function onTagChange(tag: string) {
  ...
  sendMarketTrack('market_tag_change', '切换行情标签', { tag }, '标签筛选', '.filter-tag.active')
}
function onSearch() {
  page.value = 1
...
function onRefresh() {
  ...
  sendMarketTrack('market_refresh', '刷新市场数据', { filterType: filterType.value, page: page.value }, '刷新按钮', '.refresh-market-btn')
}
function openBuyModal(stock: any) {
  buyStock.value = stock
  showBuyModal.value = true
  sendMarketTrack('market_buy_click', '点击买入', { tsCode: stock.tsCode, name: stock.name }, '买入按钮', '.buy-btn')
}
function onPageClick(p: number) {
  fetchMarketData(filterType.value, p)
  sendMarketTrack('market_page_change', '切换市场分页', { page: p }, '分页按钮', '.page-btn.active')
}
onMounted(() => {
  fetchMarketData('全部')
})
