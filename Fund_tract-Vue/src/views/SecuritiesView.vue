  ...
  <div class="securities-page">
    ...
    <div class="filter-section">
      <div class="filter-row">
        ...
        <div class="filter-group">
          <label>基金代码</label>
          <input v-model="filters.name" type="text" placeholder="请输入基金代码" />
        </div>
        <div class="filter-group">
          <label>市场类型</label>
...
function resetFilters() {
  ...
  fetchStockData(1)
  sendSecuritiesTrack('securities_reset', '重置基金信息筛选', {}, '重置按钮', '.btn-default')
}
function exportData() {
  // 可根据实际需求导出数据
  alert('导出功能开发中...')
  sendSecuritiesTrack('securities_export', '导出基金信息', { ...filters.value }, '导出按钮', '.btn-link')
}
function showDetail(stock: Stock) {
  detailTsCode.value = stock.tsCode
  showPanel.value = true
  sendSecuritiesTrack('securities_detail_click', '点击查看基金详情', { tsCode: stock.tsCode, name: stock.name }, '详情按钮', '.action-btn')
}
function onPageClick(p: number) {
  fetchStockData(p)
  sendSecuritiesTrack('securities_page_change', '切换基金信息分页', { page: p }, '分页按钮', '.page-btn.active')
}
onMounted(() => {
  fetchStockData(1)
})
